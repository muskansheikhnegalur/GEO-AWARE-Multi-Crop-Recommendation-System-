...
def load_default_data():
    return pd.read_csv("live_mandi.csv")
...
if st.button("🚀 Recommend Best Crop"):
    ...
    data["Crop_clean"] = data["Crop"].str.strip().str.lower()
    crop_clean = crop.strip().lower()
    st.write("🔍 Predicted crop:", crop)
    st.write("🧾 Available crops in CSV:", data["Crop_clean"].unique())

    match = data[data["Crop_clean"].str.contains(crop_clean)]
...
# 📊 Prepare Chart Data
chart_data = top5[["Crop", "ProfitPerHa"]]
chart = alt.Chart(chart_data).mark_bar(color="#66bb6a").encode(
    x=alt.X("Crop", sort="-y"),
    y="ProfitPerHa",
    tooltip=["Crop", "ProfitPerHa"]
...
with col1:
    with st.expander("📈 Top 5 Profitable Crops"):
        st.table(top5[["Crop", "ProfitPerHa"]].style.format({"ProfitPerHa": "₹{:,.2f}"}))
